  ...
  <div v-show="!showBoot" ref="root" class="hq">
    ...
    <header ref="headerEl" class="hdr">
      ...
      <div class="hdr-meta">
        ...
        SYSTEME ACTIF &nbsp;·&nbsp;
        {{ user?.username || "ANONYMOUS" }} &nbsp;·&nbsp; {{ sessionTime }}
      </div>
    </header>
    ...
    <div ref="gridEl" class="grid">
      ...
      <div ref="c0" class="card">
        ...
        <div class="c-hd">
          <span class="c-dot"></span>系统状态<span class="badge ok"
            >ONLINE</span
          >
        </div>
        <div class="c-bd">
          <div class="kv">
            ...
            ><span class="kv-v ok">● ACTIVE</span>
          </div>
          <div class="kv">
            <span class="kv-k">▸ 会话</span
            ><span class="kv-v">{{ sessionTime }}</span>
          </div>
          <div class="prog-group">
            <div class="pg-row">
              <span>系统负载</span><span ref="ldNum" class="pg-pct">0%</span>
            </div>
            <div class="pg-track"><div ref="ldBar" class="pg-fill"></div></div>
          </div>
          <div class="prog-group">
            <div class="pg-row">
              <span>安全指数</span><span ref="scNum" class="pg-pct ok">0%</span>
            </div>
            <div class="pg-track">
              <div ref="scBar" class="pg-fill ok"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div ref="c3" class="card">
        ...
        <div class="c-bd data-bd">
          ...
          <div v-else-if="loadingDetails" class="ph">
            扫描中<span class="dots">...</span>
          </div>
    ...
    <div ref="tickerEl" class="ticker">
      <div ref="tickInner" class="tick-inner">
        BUFFOTTE v2.0 &nbsp;•&nbsp; CS2 SKIN INTELLIGENCE &nbsp;•&nbsp; POWERED
        BY AI &nbsp;•&nbsp; DATA SECURED &nbsp;•&nbsp; ALL SYSTEMS NOMINAL
        &nbsp;•&nbsp;&emsp; BUFFOTTE v2.0 &nbsp;•&nbsp; CS2 SKIN INTELLIGENCE
        &nbsp;•&nbsp; POWERED BY AI &nbsp;•&nbsp; DATA SECURED &nbsp;•&nbsp; ALL
        SYSTEMS NOMINAL &nbsp;•&nbsp;&emsp;
      </div>
    </div>
  </div>
</template>
...
const scNum = ref(null);
const cpuBar = ref(null);
const memBar = ref(null);
const netBar = ref(null);
...
const ringOffset = computed(() => {
  ...
  const pct = Math.min(
    userDetails.value.tracked_count /
      Math.max(userDetails.value.track_permit, 1),
    1,
  );
  return CIRC * (1 - pct);
});
...
const pRows = computed(() => {
  ...
  if (userDetails.value) {
    rows.push({
      k: "等级",
      v: `Lv.${userDetails.value.level} · ${userDetails.value.level_name}`,
    });
...
const initGSAP = () => {
  _ctx = gsap.context(() => {
    ...
    tl.call(() => {
      ...
      setTimeout(() => {
        if (ldNum.value) ldNum.value.textContent = "76%";
      }, 1300);
      gsap.to(scBar.value, {
        width: "99%",
        duration: 1.4,
      ...
      setTimeout(() => {
        if (scNum.value) scNum.value.textContent = "99%";
      }, 1600);
      gsap.to(cpuBar.value, {
        width: "82%",
        duration: 1.1,
        ease: "power2.out",
      });
      gsap.to(memBar.value, {
        width: "58%",
        duration: 1.1,
        ease: "power2.out",
        delay: 0.1,
      });
      gsap.to(netBar.value, {
        width: "100%",
        duration: 0.8,
        ease: "power2.out",
      });
...
.kv-v.ok {
  color: #00ff8b;
  text-shadow: 0 0 8px rgba(0, 255, 139, 0.45);
}
.ok {
...
.dots {
  animation: dotanim 1.2s step-end infinite;
}
@keyframes dotanim {
  0%,
  100% {
