...
<style scoped>
.kline-container{width:100%;height:100vh;background-color:#000;display:flex;flex-direction:column;justify-content:center;align-items:center;padding:20px}
.controls{display:flex;gap:20px;margin-bottom:20px;align-items:center}
.switch-container{display:flex;align-items:center;gap:10px;color:#00ff8b;font-family:'Source Code Pro',monospace}
.switch{position:relative;display:inline-block;width:50px;height:24px}
.switch input{opacity:0;width:0;height:0}
.slider{position:absolute;cursor:pointer;top:0;left:0;right:0;bottom:0;background-color:#333;-webkit-transition:.4s;transition:.4s}
.slider:before{position:absolute;content:"";height:16px;width:16px;left:4px;bottom:4px;background-color:white;-webkit-transition:.4s;transition:.4s}
input:checked + .slider{background-color:#00ff8b}
input:focus + .slider{box-shadow:0 0 1px #00ff8b}
input:disabled + .slider{background-color:#222;cursor:not-allowed}
input:checked + .slider:before{-webkit-transform:translateX(26px);-ms-transform:translateX(26px);transform:translateX(26px)}
.slider.round{border-radius:24px}
.slider.round:before{border-radius:50%}
.chart{width:100%;height:85%}
</style>
