...
.items-grid-container {
  display: grid;
  grid-template-columns: 0.8fr 2.2fr; /* 左侧搜索、右侧价格和K线 */
...
@media (max-width: 900px) {
  .items-grid-container {
    grid-template-columns: 1fr; /* 小屏幕下所有区域堆叠 */
...
.search-results {
  margin-top: 24px; /* 调整与搜索框的间距 */
  background: rgba(0, 0, 0, 0.4);
  border: 1px solid #00ff41;
  border-radius: 8px;
  overflow: hidden;
  flex-grow: 1; /* 让搜索结果列表填充剩余空间 */
  display: flex;
  flex-direction: column;
}
...
.price-grid {
  ...
  grid-auto-rows: minmax(min-content, max-content); /* 行高自适应 */
  gap: 12px;
